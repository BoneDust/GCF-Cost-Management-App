...
//     if(verification.isValidAdmin(req.headers.token))
//     {
//         if (req.query.search)
//         {
//             const filterParams = {
    ...
    if (user.email && user.password)
    {
        ...
        dynamoDb.query(params, (error, result) => {
            ...
            if (result.Count > 0)
            {
                    const responseStatusCode = 200;
                    const token = uuid.v1();
                    const type = result.Items[0].privilege;// or type =  result.Items[item].privilege ... for (var item in result.Items)
                    const response = {
                        access_token: token,
                    };
                    verification.saveUsertoken(token, type);
                    res.status(responseStatusCode).json(response);
