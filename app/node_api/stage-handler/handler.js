...
app.get('/stages/:project_id', (req, res) => {
...
app.post('/stages', (req, res) => {
    verification.isValidUser(req.headers.token).then(isValid => {
        if (isValid) {
            const stage = req.body; if (stage.project_id && stage.stage_name && stage.description && stage.status && stage.before_pic_url && stage.start_date && estimated_duration) {
                const params = {
                    TableName: STAGES_TABLE,
                    Item: {
